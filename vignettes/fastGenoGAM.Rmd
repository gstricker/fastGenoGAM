...
```{r plot, fig.cap = "The fit of input (upper) and IP (lower) for chrI:45000-55000", fig.wide = TRUE}
par(mfrow = c(2,1))
plot(fits(result)$chrI[,"s(x)"][idx], type = "l", ylab = "log-fit")
lines(upper_input, lty = 2)
lines(lower_input, lty = 2)

plot(fits(result)$chrI[,"s(x):genotype"][idx], type = "l", ylab = "log-fit")
